...
This directory now contains a **comprehensive token-saving system** designed to help agents (Claude or any other AI) work efficiently on the Abertamente Vue 3 frontend project.
...
- **Other skills** — Diagram processor, workflows (unchanged)
...
→ Read: `.agents/rules/project-overview.md` or `.agents/memory/01-architecture.md` (detailed)
...
- ✅ Composables for global state, local `ref` for components
- ✅ Component props always typed: `defineProps<{}>()`
- ✅ SCSS `@use` only, no `@import`
- ✅ No Options API — use `<script setup>` only
- ✅ All language files (PT, EN, ES) must match i18n keys
- ✅ Form validation uses vee-validate with zod
- ✅ Zero mock data in production, double verification for delete actions
...
2. Foundation: `.agents/rules/project-overview.md` (3 min)
...
2. **Tell them to read**: `.agents/rules/project-overview.md` + their role's cache files
...
**System created**: 2026-05-12  
**Cache version**: 1.1 (Abertamente Vue 3 Vite)  
