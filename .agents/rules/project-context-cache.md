...
### 2. Project Overview (.agents/rules/project-overview.md)
Quick reference for:
- Tech stack summary
...
→ Read project-overview.md (summary)
...
| **Why Vue 3/Tailwind/TypeScript** | `.agents/memory/04-tech-decisions.md` | 5KB |
| **Quick tech reference** | `.agents/rules/project-overview.md` | 3KB |
...
3. Grep for example keys in src/locales/ files

### "I need to test a component"
1. Read `conventions.md` section on testing
2. Find existing test in `hot_paths.md`
3. Grep for `.cy.ts` or `.spec.ts` examples
...
- ✅ All language files (pt, en, es) must match keys
- ✅ Environment variables never exposed to client via console.log
- ✅ Vee-validate with Zod for all forms
...
### @pm (Product Manager)
- Read `.agents/rules/project-overview.md` for tech context
...
- Read `.agents/rules/developer-workflow-environment.md`
- Read `tech_decisions.md` for build/deploy constraints
- Node 22+ required
- Use standard Vite/Vue npm scripts
