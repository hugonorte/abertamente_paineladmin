...
## Instructions
1. **Stack Detection**: Inspect the rules in `.agents/rules/` and the files in `src/` to figure out what stack is being used.

2. **Install Dependencies**: Use your native terminal to navigate into `src/` and run `npm install`, `pip install -r requirements.txt`, or whatever is appropriate!
