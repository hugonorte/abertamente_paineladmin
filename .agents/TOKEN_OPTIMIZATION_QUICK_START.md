...
.agents/rules/project-overview.md     ← Quick reference (read this first!)
```
...
# or "src/views/" depending on structure

# Find composable/store
Tool: grep_search (Pattern: "use[NAME]", Path: "src/")

# Find usage
Tool: grep_search (Pattern: "MyComponent", Path: "src/")

# Find test
Tool: grep_search (Pattern: "[NAME]", Path: "cypress/", Includes: ["*.cy.ts"])
```
...
| **"Where is ComuneroModel?"** | 03-hot-paths.md | `grep_search: ComuneroModel` | 1K |
| **"How do I write TypeScript?"** | 02-conventions.md | - | 3K |
| **"What's the Composables setup?"** | 01-architecture.md | `grep_search: defineStore in composables/` | 3K |
...
| **"I need to see an example"** | 04-tech-decisions.md | `grep_search: MyPattern in src/` | 2K |
...
3. Grep: existing tests in cypress/ (1KB)
4. Write: new tests (0KB)
5. Run: tests with npm run cy:open (0KB)
...
3. Run: npm run build / npm run type-check (0KB)
...
**Task**: "Add i18n to ComuneroDetails component"
...
6. **Validate fast** — Run linters/type checks, don't read test code to understand

---

## Checklist: Before Starting Any Task

- [ ] Have I read .agents/rules/project-overview.md? (yes = 3K tokens saved)
...
- [ ] Are type checks passing? (yes = 0 tokens on debugging)
...
```
.agents/rules/project-overview.md     ← START HERE
...
1. **Right now**: Read `.agents/rules/project-overview.md` (3 minutes)
...
6. **Run type-check/tests**: Validate
7. **Commit**: Done!
